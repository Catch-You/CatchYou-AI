...
model_path="SujinHwang/criminal-sketch-lora-v2-2"
...
def save_images(images, file_name):
    # s3 = boto3.resource('s3', aws_access_key_id= "your AWS access key", aws_secret_access_key= "your AWS secret key")
    for i, image in enumerate(images):
        pil_image = Image.fromarray(image)
        b = io.BytesIO()
        pil_image.save(b, 'png')
        im_bytes = b.getvalue()
        s3.Bucket('diffusion-ml').put_object(Key=f'{file_name}.png', Body=im_bytes, ContentType='image/png')

        image_path = f"/home/ubuntu/project/Image/{file_name}.png"
        pil_image.save(image_path)


    return f"https://diffusion-ml.s3.ap-northeast-2.amazonaws.com/{file_name}.png"
